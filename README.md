...
<p align="center">
  <img src="./assets/matrix-code-banner.png" alt="Matrix Code" width="900">
</p>
...
The portable launcher stores configuration, sessions, state and cache under the extracted `.matrix` directory. It discovers the executable, Voice helper and model relative to its own folder. OmniRoute and its Node.js runtime are included, so the end user does not need globally installed Bun, Node.js, npm or Python.
...
Matrix Code uses the existing OpenAI-compatible provider implementation. The Windows Portable starts the bundled OmniRoute at `http://127.0.0.1:20128/v1` when no healthy external instance exists. Its local authentication is configured automatically without printing or passing credentials on the command line.

**Matrix Coding Free (Direct)** maps to OmniRoute's `auto/coding:free` route. **Matrix Coding Reliable** and **Matrix Free Auto** remain available through the Matrix router.

Matrix Code and OmniRoute cooperate on routing and resilience. OmniRoute handles provider-level health, quotas, retries and routing capabilities, while Matrix Code provides its own reliable candidate and fallback behavior where applicable. The normal model dialog still allows advanced manual provider/model selection.
...
Windows releases bundle a PyInstaller-built Voice helper, its Python runtime, native audio dependencies and a local faster-whisper model. Python is a build-time dependency only; end users do not install it.

The source helper remains usable for development through:

* `MATRIX_VOICE_HELPER`
* `MATRIX_VOICE_MODEL_DIR`
* `MATRIX_VOICE_PYTHON`
* `MATRIX_VOICE_LANGUAGE`

Press `F9` to cycle the TUI language between English, Portuguese (Portugal) and Portuguese (Brazil).

## Updating Matrix Code

When updating the Windows Portable edition, preserve the `.matrix` directory. It contains sessions, configuration, providers, preferences and protected local credentials.

Do not manually copy API keys, and do not delete the previous version's data until the new version has been validated.

See the complete procedure in [docs/UPDATING.md](docs/UPDATING.md).
...
powershell -File .\script\build-windows-distribution.ps1
```

Outputs are written under `packages/opencode/dist/matrix-release` and include SHA-256 checksums.

The `matrix-windows-release` GitHub Actions workflow builds the same assets and attaches them to tagged GitHub Releases.

## Upstream compatibility and attribution

Matrix Code is based on [OpenCode](https://github.com/anomalyco/opencode) and preserves the upstream MIT license and internal compatibility.

Copyright notices and the repository [LICENSE](LICENSE) remain in distributions.

Matrix Code branding does not imply ownership of OpenCode or of any external AI provider.

```

Também corrigi uma inconsistência importante: antes dizia que **“Matrix Code does not duplicate routing logic”**, mas o próprio `Matrix Coding Reliable` tem lógica de fallback/resiliência no Matrix. Deixei a descrição mais fiel ao funcionamento atual.

E mudei `## Atualizar o Matrix Code` para `## Updating Matrix Code`, porque este é o `README.md` em inglês.

A imagem continuará quebrada no preview do **GitHub** enquanto `assets/matrix-code-banner.png` existir só no seu PC. Quando fizermos o próximo commit/push, precisamos incluir **o README e a imagem juntos**.
```

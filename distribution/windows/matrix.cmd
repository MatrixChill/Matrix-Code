...
rem
rem  With no arguments, launches matrix.ps1 invisibly so the desktop
...
rem  With CLI arguments, the orchestrator and child stay in the caller's console.
rem
...
rem Only a zero-argument desktop launch hides the orchestrator. Keep CLI
rem stdout/stderr attached, including when the caller redirects them.
if "%~1"=="" if "%1"=="" goto desktop
set "MATRIX_TUI_WINDOW="
"%MATRIX_POWERSHELL%" -NoProfile -File "%MATRIX_PS1%" %*
exit /b %ERRORLEVEL%

...
describe("http-recorder", () => {
        ...
        interactions: [
          {
            ...
            request: {
              method: "POST",
              url: "https://example.test/path?key=REMOVED_API_KEY",
              headers: {},
              body: JSON.stringify({ nested: "REMOVED_GOOGLE_KEY" }),
            },
            response: {
              status: 200,
              headers: {},
  ...
  test("detects secret-looking values inside metadata", () => {
    expect(
      HttpRecorder.secretFindings({
        version: 1,
        metadata: { token: "REMOVED_API_KEY" },
        interactions: [],
      }),
    ...
    await run(
      Effect.gen(function* () {
        const exit = yield* Effect.exit(
          post("https://example.test/echo?api_key=secret-value", { step: 3, token: "REMOVED_API_KEY" }),
        )
        ...
        expect(message).not.toContain("REMOVED_API_KEY")
      }),
    )
  })
})
